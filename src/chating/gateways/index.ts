  ...
  implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect
{
  ...
  // 이벤트 메세지 정리
  @SubscribeMessage(ChatingConfigSocketSubTypes.JOIN_ROOM)
  async onJoinRoomAsync(
    ...
    this.server
      .to(client.id)
      .emit(
        ChatingConfigSocketPubTypes.GET_PROFILE,
        updatedUserCount.toString(),
      );

    client
      .to(roomId)
      .emit(
        ChatingConfigSocketPubTypes.JOINED_NEW_MEMBER,
        updatedUserCount.toString(),
      );
  }

  @SubscribeMessage(ChatingConfigSocketSubTypes.LEAVE_ROOM)
  async onLevenRoomAsync(
    ...
    client
      .to(roomId)
      .emit(ChatingConfigSocketPubTypes.LEAVED_EXISTING_MEMBER, nickName);

    this.disconnectClient(client);
  }

  @SubscribeMessage(ChatingConfigSocketSubTypes.SEND_MESSAGE)
